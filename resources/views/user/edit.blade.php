                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('user_update', $user->id) }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select name="notification" id="notification" class="form-control @error('notification') is-invalid @enderror">
                                    <option value="0" {{ (old('notification', $user->notification) == 0 ? "selected":"") }}>Off</option>
                                    <option value="1" {{ (old('notification', $user->notification) == 1 ? "selected":"") }}>On</option>
                                </select>
                                @error('notification')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
